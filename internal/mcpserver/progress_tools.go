...
func registerSessionProgress(srv *mcp.Server, client *api.Client) {
	...
	description := "Per-requirement progress for one intent session: coverage on the ladder no_test -> not_run -> failing -> verified, whether the deciding tests came from this session, overall readiness, and next actions. Starting, refining, and closing a session remain CLI-only because they require local git and session state."
...
type riskAcceptanceArgs struct {
	RequirementRefs []string `json:"requirement_refs" jsonschema:"Requirement UUIDs or CODE-N refs covered by this decision (required, non-empty)."`
...
type recordRiskAcceptancesArgs struct {
	...
	ProposedTestRequirementRefs []string             `json:"proposed_test_requirement_refs,omitempty" jsonschema:"Requirement UUIDs or CODE-N refs whose connected debt is deferred to a proposed test."`
}
	...
	mcp.AddTool(srv, &mcp.Tool{
		...
		for i, acceptance := range args.Acceptances {
			acceptances[i] = api.SessionRiskAcceptance{
				RequirementRefs: acceptance.RequirementRefs,
				Criterion:       acceptance.Criterion,
				Evidence:        acceptance.Evidence,
				FollowUp:        acceptance.FollowUp,
			}
		}
