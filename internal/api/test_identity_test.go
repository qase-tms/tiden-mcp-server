...
func TestListTestIdentitiesPage(t *testing.T) {
	rowA := `{"id":"t1","productId":"p","branchStatus":"clean","kind":"case","title":"T1","seqNum":1,"tags":[],"signature":"sig1","framework":"go","filePath":"a.go","layer":"unit","isAutomated":true}`
	rowB := `{"id":"t2","productId":"p","branchStatus":"clean","kind":"case","title":"T2","seqNum":2,"tags":[],"signature":"sig2","framework":"go","filePath":"b.go","layer":"unit","isAutomated":true}`
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				calls++
				if r.Method != "GET" || r.URL.Path != "/v1/products/p/tests" {
			...
			got, err := New(server.URL, "token").ListTestIdentitiesPage(context.Background(), "p", "", tc.size, tc.token)
